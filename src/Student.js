...
class Student {
  ...
  constructor(id, name, studentClass, grades = {}) {
    if (!id || !name || !studentClass) {
      throw new Error('ID, name, dan kelas wajib diisi.');
    }
    this.id = id;
    this.name = name;
  ...
  addGrade(subject, score) {
    if (!subject) throw Error('Nama mata pelajaran wajib diisi.');
  ...
  getAverage() {
    ...
    const total = Object.values(this.grades).reduce((sum, val) => sum + val, 0);
   ...
   */
  getGradeStatus() {
    const status = this.getAverage();
    return status >= 75 ? 'Lulus' : 'Tidak Lulus';
  }
  ...
  displayInfo() {
    ...
    if (Object.keys(this.grades).length === 0) {
      ...
      for (const [sub, score] of Object.entries(this.grades)) {
